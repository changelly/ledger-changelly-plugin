...
nano_models.forEach(function (model) {
  test(
    '[Nano ' + model.letter + '] Sell token for token uniswap',
    zemu(model, async (sim, eth) => {
      const contract = new ethers.Contract(contractAddr, abi);

      // Constants used to create the transaction
      const RECIPIENT = '0xEC89F1E142DFBDDbe30487fF5026c7427023e982';
      const SELL_AMOUNT = BigNumber.from('18434430000000000000');
      ...
      const { data } = await contract.populateTransaction.sellTokenForTokenToUniswapV3(
        OFFSET,
        SELL_AMOUNT,
        MIN_BUY_AMOUNT,
        RECIPIENT
      ...
      await sim.navigateAndCompareSnapshots('.', model.name + 'sell-token-for-token-uniswap', [
        right_clicks,
        0,
      ]);
