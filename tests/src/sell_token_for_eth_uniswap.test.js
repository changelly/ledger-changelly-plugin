...
nano_models.forEach(function (model) {
  test(
    '[Nano ' + model.letter + '] Sell token for ETH uniswap',
    zemu(model, async (sim, eth) => {
      const contract = new ethers.Contract(contractAddr, abi);

      // Constants used to create the transaction
      const RECIPIENT = '0xEC89F1E142DFBDDbe30487fF5026c7427023e982';
      const MIN_BUY_AMOUNT = BigNumber.from('122301200000000000');
      const SELL_AMOUNT = BigNumber.from('32454343000000');
      ...
      const { data } = await contract.populateTransaction.sellTokenForEthToUniswapV3(
        OFFSET,
        SELL_AMOUNT,
        MIN_BUY_AMOUNT,
        RECIPIENT
      ...
      await sim.navigateAndCompareSnapshots('.', model.name + 'sell-token-for-eth-uniswap', [
        right_clicks,
        0,
      ]);
